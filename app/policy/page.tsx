  ...
  const autonomyModes = [
    {
      ...
      label: "Observe Only",
      icon: Eye,
      desc: "Ingest telemetry, no AI diagnosis.",
    },
    {
      id: "RECOMMEND_ONLY",
      label: "Recommend Only",
      icon: MessageSquare,
      desc: "Diagnose and propose, but no execution pathways.",
    },
    {
      id: "APPROVAL_REQUIRED",
      label: "Approval Required",
      icon: ShieldCheck,
      desc: "Require human operator sign-off at Veto Gate.",
    },
    {
      id: "FULL_AUTONOMY",
      label: "Full Autonomy",
      icon: Zap,
      desc: "AI executes if confidence & shadow sandbox pass.",
    },
  ];
      ...
      <div className="flex items-center justify-between border-b border-zinc-800 pb-4">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">
            Configure AI operational boundaries, safety gates, and confidence
            thresholds.
          </p>
        </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      />
                      <span
                        className={`font-bold ${isActive ? "text-zinc-100" : "text-zinc-400"}`}
                      >
                        {mode.label}
                      </span>
                    </div>
                    <p
                      className={`text-xs ${isActive ? "text-zinc-300" : "text-zinc-500"}`}
                    >
                      {mode.desc}
                    </p>
                  </button>
            ...
            {autonomyMode === "FULL_AUTONOMY" && (
              <div className="mt-4 p-3 bg-orange-500/10 border border-orange-500/20 rounded-md flex items-start gap-3">
                ...
                <p className="text-xs text-orange-400/90 leading-relaxed">
                  <strong>Warning:</strong> Full Autonomy allows the AI to
                  mutate production state without human intervention, provided
                  confidence thresholds and Shadow Sandbox validation pass.
                  Ensure Prohibited Targets are strictly defined.
                </p>
              </div>
            )}
          </section>
          ...
          <section className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
            ...
            <div className="space-y-6">
              {[
                {
                  key: "diagnosis",
                  label: "Diagnosis Certainty Minimum",
                  value: thresholds.diagnosis,
                },
                {
                  key: "action",
                  label: "Action Success Probability",
                  value: thresholds.action,
                },
                {
                  key: "sandbox",
                  label: "Shadow Sandbox Pass Rate",
                  value: thresholds.sandbox,
                },
              ].map((slider) => (
                <div key={slider.key}>
                  <div className="flex justify-between text-sm font-medium mb-2">
                    ...
                    <span
                      className={`font-mono ${slider.value < 85 ? "text-orange-400" : "text-emerald-400"}`}
                    >
                      {slider.value}%
                    </span>
                  </div>
                  ...
                  {slider.value < 85 && (
                    <p className="text-xs text-orange-500 mt-1 flex items-center gap-1">
                      <AlertTriangle className="w-3 h-3" /> Risk of unsafe
                      execution. Thresholds below 85% are not recommended.
                    </p>
                  )}
                </div>
              ))}
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-3">
              {Object.entries(allowedActions).map(([action, isAllowed]) => (
                ...
                >
                  ...
                  <button
                    onClick={() =>
                      toggleAction(action as keyof typeof allowedActions)
                    }
                    className={`w-10 h-5 rounded-full relative transition-colors ${isAllowed ? "bg-emerald-500" : "bg-zinc-700"}`}
                  >
                    <span
                      className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full transition-transform ${isAllowed ? "translate-x-5" : "translate-x-0"}`}
                    />
                  </button>
                </div>
              ))}
          ...
          <section className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 border-t-4 border-t-red-500">
            ...
            <p className="text-xs text-zinc-500 mb-4 leading-relaxed">
              Resources explicitly blocked from AI mutation regardless of
              confidence scores or autonomy modes.
            </p>
            <div className="space-y-2">
              {prohibitedTargets.map((target) => (
                <div
                  key={target}
                  className="flex items-center gap-3 p-3 bg-red-950/10 border border-red-900/30 rounded-lg"
                >
